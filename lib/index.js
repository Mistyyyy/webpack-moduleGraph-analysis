...
module.exports = class ModuleGraphAnalysis {
  ...
  getSource(connection, type) {
    if (connection) {
      if (connection.originModule) {
        if (type === 'raw')
          return this.getPath(connection.originModule.rawRequest)
  ...
  getTarget(connection, type) {
    if (connection) {
      if (type === 'raw') return this.getPath(connection.module.rawRequest)
    ...
    return this.toArray(outgoingConnections)
      .map(connection => {
        return {
          ...
          shape: 'CircleNode',
          data: this.getParentModule(connection)
        }
      })
  }
    ...
    return this.toArray(incomingConnections)
      .map(connection => {
        return {
          ...
          shape: 'CircleNode',
          data: this.getChildModule(connection)
        }
      })
  }
